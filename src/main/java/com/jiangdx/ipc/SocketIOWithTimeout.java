...
abstract class SocketIOWithTimeout {
	...
	static void checkChannelValidity(Object channel) throws IOException {
		if (channel == null) {
			/*
			 * Most common reason is that original socket does not have a
			 * channel. So making this an IOException rather than a
			 * RuntimeException.
			 */
	...
	/**
	 * Performs one IO and returns number of bytes read or written. It waits up
	 * to the specified timeout. If the channel is not read before the timeout,
	 ...
	 *            SelectionKey.OP_READ while reading and SelectionKey.OP_WRITE
	 *            while writing.
	 * 
	...
	static void connect(SocketChannel channel, SocketAddress endpoint,
		...
		try {
			...
			// javadoc for SocketChannel.connect() says channel should be
			// closed.
			try {
				channel.close();
	...
	private static class SelectorPool {
		 ...
		 * selectors. It also removes any cached selectors that are idle for a
		 * few seconds.
		 * 
		 * @param channel
		 * @param ops
		...
		/**
		 * Takes one selector from end of LRU list of free selectors. If there
		 * are no selectors awailable, it creates a new selector. Also invokes
		 * trimIdleSelectors().
		 * 
		 * @param channel
		 * @return
		...
		private synchronized SelectorInfo get(SelectableChannel channel)
			...
			if (pList == null) {
				// LOG.info("Creating new ProviderInfo : " +
				// provider.toString());
		...
		/**
		 * puts selector back at the end of LRU list of free selectos. Also
		 * invokes trimIdleSelectors().
		 * 
		 * @param info
		 */
		 ...
		 * traverse the whole list, just over the one that have crossed the
		 * timeout.
		 */
